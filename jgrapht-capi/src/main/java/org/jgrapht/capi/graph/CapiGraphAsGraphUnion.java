 ...
 * 
 * <p>
 * Read-only union of two graphs: G<sub>1</sub> and G<sub>2</sub>. If
 ...
 * and E<sub>2</sub>. A {@link WeightCombiner} in order to calculate edge
 * weights.
 * 
 ...
 */
public class CapiGraphAsGraphUnion<V, E> extends AbstractGraph<V, E> implements CapiGraph<V, E>, Serializable {
	...
	private static final String NOT_SUPPORTED_ON_UNION_GRAPH = "not supported on union graphs";

	private final CapiGraph<V, E> g1;
	private final GraphType type1;
	private final CapiGraph<V, E> g2;
	 ...
	 */
	public CapiGraphAsGraphUnion(CapiGraph<V, E> g1, CapiGraph<V, E> g2, WeightCombiner operator) {
		this.g1 = Objects.requireNonNull(g1);
		if (!g1.getType().isDirected() && !g1.getType().isUndirected()) {
			throw new IllegalArgumentException("Graph must be directed or undirected");
		}
		this.type1 = g1.getType();

		this.g2 = Objects.requireNonNull(g2);
		if (!g2.getType().isDirected() && !g2.getType().isUndirected()) {
			throw new IllegalArgumentException("Graph must be directed or undirected");
		}
		...
		this.type = builder.allowSelfLoops(type1.isAllowingSelfLoops() || type2.isAllowingSelfLoops())
				.allowMultipleEdges(true).weighted(true).modifiable(false).build();
	}

	/**
	 * Construct a new graph union. The union will use the
	 * {@link WeightCombiner#SUM} weight combiner.
	 * 
	 * @param g1 the first graph
	 * @param g2 the second graph
	 */
	public CapiGraphAsGraphUnion(CapiGraph<V, E> g1, CapiGraph<V, E> g2) {
	...
	public Set<E> getAllEdges(V sourceVertex, V targetVertex) {
		...
		if (inG1 && inG2) {
			return new UnmodifiableUnionSet<>(g1.getAllEdges(sourceVertex, targetVertex),
	...
	public Set<E> incomingEdgesOf(V vertex) {
		...
		if (inG1 && inG2) {
			return new UnmodifiableUnionSet<>(g1.incomingEdgesOf(vertex), g2.incomingEdgesOf(vertex));
		} else if (inG1) {
	...
	public Set<E> outgoingEdgesOf(V vertex) {
		...
		if (inG1 && inG2) {
			return new UnmodifiableUnionSet<>(g1.outgoingEdgesOf(vertex), g2.outgoingEdgesOf(vertex));
		} else if (inG1) {
	...
	public ExternalRef toExternalRef(PointerBase ptr) {
		return g1.toExternalRef(ptr);
	}

	@Override
	public GraphAnyStore<V, E> getStore() {
		throw new UnsupportedOperationException(NOT_SUPPORTED_ON_UNION_GRAPH);
	}
