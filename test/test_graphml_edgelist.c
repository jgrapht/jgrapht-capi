...
int main() {
    ...
    void *edgelist;
    jgrapht_capi_ii_import_edgelist_attrs_string_graphml(thread, expected, 1, NULL, NULL, 
        &edgelist);
    assert(jgrapht_capi_error_get_errno(thread) == 0);
    ...
    // no attrs
    jgrapht_capi_xx_import_edgelist_noattrs_string_graphml(thread, expected, 1, &edgelist);
    assert(jgrapht_capi_error_get_errno(thread) == 0);
