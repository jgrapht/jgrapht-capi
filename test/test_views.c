...
int main() { 
    ...
    void *g1;
    jgrapht_capi_xx_graph_as_undirected(thread, g, &g1);

    assert(jgrapht_capi_xx_graph_is_directed(thread, g1, &flag) == 0);
    assert(flag == 0);
    assert(jgrapht_capi_xx_graph_is_undirected(thread, g1, &flag) == 0);
    assert(flag == 1);
    ...
    jgrapht_capi_xx_graph_as_edgereversed(thread, g, &g1);
    int v;
    jgrapht_capi_ii_graph_edge_source(thread, g1, e12, &v);
    assert(v == v2);
    jgrapht_capi_ii_graph_edge_target(thread, g1, e12, &v);
    assert(v == v1);
    jgrapht_capi_handles_destroy(thread, g1);

    jgrapht_capi_xx_graph_as_unmodifiable(thread, g, &g1);
    jgrapht_capi_ii_graph_add_edge(thread, g1, v1, v5, NULL);
    assert(jgrapht_capi_error_get_errno(thread) != 0);
