 ...
 */
package org.jgrapht.capi.custom.traverse;
 ...
 * <p>
 * Every vertex has an implicit label (they aren't used explicitly in order to
 * reduce time and memory complexity). When some vertex is returned by this
 * iterator, its index is the number of vertices in this graph minus number of
 * already returned vertices. For a given vertex v its label is a concatenation
 * of indices of already returned vertices, that were also its neighbours, with
 * some separator between them. For example, 7#4#3 is a valid vertex label.
 * <p>
 * Iterator chooses vertex with lexicographically largest label and returns it.
 * It breaks ties arbitrarily. For more information on lexicographical BFS see
 * the following article: Corneil D.G. (2004) <a href=
 * "https://pdfs.semanticscholar.org/d4b5/a492f781f23a30773841ec79c46d2ec2eb9c.pdf">
 * <i>Lexicographic Breadth First Search – A Survey</i></a>. In: Hromkovič J.,
 * Nagl M., Westfechtel B. (eds) Graph-Theoretic Concepts in Computer Science.
 * WG 2004. Lecture Notes in Computer Science, vol 3353. Springer, Berlin,
 * Heidelberg; and the following paper:<a href=
 * "http://www.cse.iitd.ac.in/~naveen/courses/CSL851/uwaterloo.pdf"><i>CS 762:
 * Graph-theoretic algorithms. Lecture notes of a graduate course. University of
 * Waterloo</i></a>.
 * <p>
...
public class LexBreadthFirstIterator<V, E> extends AbstractGraphIterator<V, E> {
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Always returns true since this iterator doesn't care about connected
	 * components.
	 */
	@Override
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Trying to disable the cross components nature of this iterator will result
	 * into throwing a {@link IllegalArgumentException}.
	 */
	@Override
	...
	/**
	 * Computes and returns neighbours of {@code vertex} which haven't been visited
	 * by this iterator.
	 *
	 * @param vertex the vertex, whose neighbours are being explored.
	 * @return neighbours of {@code vertex} which have yet to be visited by this
	 *         iterator.
	 */
	...
	/**
	 * Data structure for performing lexicographical breadth-first search. Allows to
	 * add and retrieve vertices from buckets, update their buckets after a new
	 * vertex has been added to the LexBFS order. Labels aren't used explicitly,
	 * which results in time and space optimization.
	 *
	 * @author Timofey Chudakov
	 */
	class BucketList {
		...
		private Bucket head;
		/**
		 * Map for mapping vertices to buckets they are currently in. Is used for
		 * finding the bucket of the vertex in constant time.
		 */
		private Map<V, Bucket> bucketMap;

		/**
		 * Creates a {@code BucketList} with a single bucket and all specified
		 * {@code vertices} in it.
		 *
		 * @param vertices the vertices of the graph, that should be stored in the
		 *                 {@code head} bucket.
		 */
		 ...
		 *
		 * @param vertex the vertex whose presence in some {@code Bucket} in this
		 *               {@code BucketList} is checked.
		 * @return <tt>true</tt> if there exists a bucket with {@code vertex} in it,
		...
		/**
		 * For every bucket B in this {@code BucketList}, which contains vertices from
		 * the set {@code
		 * vertices}, creates a new {@code Bucket} B' and moves vertices from B to B'
		 * according to the following rule: $B' = B\cap vertices$ and $B = B\backslash
		 * B'$. For every such {@code Bucket} B only one {@code Bucket} B' is created.
		 * If some bucket B becomes empty after this operation, it is removed from the
		 * data structure.
		 *
		...
		/**
		 * Plays the role of the container of vertices. All vertices stored in a bucket
		 * have identical label. Labels aren't used explicitly.
		 * <p>
		 * Encapsulates operations of addition and removal of vertices from the bucket
		 * and removal of a bucket from the data structure.
		 */
		private class Bucket {
			 ...
			 *
			 * @return vertex, that was removed from this bucket, null if the bucket was
			 *         empty.
			 */
			V poll() {
				if (vertices.isEmpty()) {
			 ...
			 *
			 * @return <tt>true</tt> if this bucket doesn't contain any elements, otherwise
			 *         false.
			 */
			boolean isEmpty() {
