...
int main() {
    ...
    void *edgelist;
    jgrapht_capi_import_edgelist_attrs_string_graphml(thread, expected, 1, NULL, NULL, 
        &edgelist);
    assert(jgrapht_capi_error_get_errno(thread) == 0);
    ...
    // no attrs
    jgrapht_capi_import_edgelist_noattrs_string_graphml(thread, expected, 1, &edgelist);
    assert(jgrapht_capi_error_get_errno(thread) == 0);
    ...
    jgrapht_capi_import_edgelist_attrs_file_graphml(thread, "test_graphml_edgelist.graphml", 1, NULL, edge_attribute, &edgelist);
    assert(jgrapht_capi_error_get_errno(thread) == 0);
    count = 0;
    jgrapht_capi_list_size(thread, edgelist, &count);
    assert (count == 3);
    ...
    jgrapht_capi_import_edgelist_noattrs_file_graphml(thread, "test_graphml_edgelist.graphml", 1, &edgelist);
    assert(jgrapht_capi_error_get_errno(thread) == 0);
    count = 0;
    jgrapht_capi_list_size(thread, edgelist, &count);
    assert (count == 3);
