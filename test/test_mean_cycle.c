...
int main() {
    ...
    jgrapht_capi_ii_graph_set_edge_weight(thread, g, 6, 5.0);
    jgrapht_capi_ii_graph_add_edge(thread, g, 3, 0, NULL);
    jgrapht_capi_ii_graph_set_edge_weight(thread, g, 7, 500.0);
    ...
    double weight;
    jgrapht_capi_ii_handles_get_graphpath(thread, cycle, &weight, &start_vertex, &end_vertex, NULL);
    assert(weight == 27);
