...
import org.jgrapht.capi.custom.partition.BipartitePartitioning;

/**
 * A two layered bipartite layout.
 * 
 * The algorithm draws a bipartite graph using straight edges. Vertices are
 * arranged along two vertical or horizontal lines. No attempt is made to
 * minimize edge crossings.
 * 
 * The order of the vertices can be adjusted by providing a vertex comparator.
 * Similarly the user can also determine the two partitions or can let the
 * algorithm compute them.
 * 
...
public class TwoLayeredBipartiteLayout2D<V, E> implements LayoutAlgorithm2D<V, E> {
	 ...
	 * 
	 * @param vertexComparator the vertex comparator, or null in order to use the
	 *                         graph ordering
	...
	/**
	 * Specify the first of the two bipartite partitions. If not provided, the
	 * algorithm computes a partitioning.
	 * 
