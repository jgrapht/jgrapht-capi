...
int main() {
    ...
    int exists;
    jgrapht_capi_list_long_contains(thread, list, 4, &exists);
    assert(exists == 0);

    int did_not_exist;
    jgrapht_capi_list_long_add(thread, list, 4, &did_not_exist);
    assert(did_not_exist);
    jgrapht_capi_list_long_contains(thread, list, 4, &exists);
    assert(exists == 1);

    jgrapht_capi_list_long_add(thread, list, 100, &did_not_exist);
    assert(did_not_exist);
    jgrapht_capi_list_long_contains(thread, list, 100, &exists);
    assert(exists == 1);

    jgrapht_capi_list_long_add(thread, list, 500, &did_not_exist);
    assert(did_not_exist);
    jgrapht_capi_list_long_contains(thread, list, 500, &exists);
    assert(exists == 1);
    ...
    jgrapht_capi_list_long_remove(thread, list, 500);
    jgrapht_capi_list_long_contains(thread, list, 500, &exists);
    assert(exists == 0);
