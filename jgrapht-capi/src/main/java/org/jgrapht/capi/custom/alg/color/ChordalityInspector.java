 ...
 */
package org.jgrapht.capi.custom.alg.color;
 ...
 * <a href="https://en.wikipedia.org/wiki/Chordal_graph">chordal</a>. A chordal
 * graph is a simple graph in which all
 * <a href="http://mathworld.wolfram.com/GraphCycle.html"> cycles</a> of four or
 * more vertices have a <a href="http://mathworld.wolfram.com/CycleChord.html">
 * chord</a>. A chord is an edge that is not part of the cycle but connects two
 * vertices of the cycle. A graph is chordal if and only if it has a <a href=
 * "https://en.wikipedia.org/wiki/Chordal_graph#Perfect_elimination_and_efficient_recognition">
 * perfect elimination order</a>. A perfect elimination order in a graph is an
 * ordering of the vertices of the graph such that, for each vertex $v$, $v$ and
 * the neighbors of $v$ that occur after $v$ in the order form a clique. This
 * implementation uses either {@link MaximumCardinalityIterator} or
 * {@link LexBreadthFirstIterator} to compute a perfect elimination order. The
 * desired method is specified during construction time.
 * <p>
 * Chordal graphs are a subset of the
 * <a href="http://mathworld.wolfram.com/PerfectGraph.html"> perfect graphs</a>.
 * They may be recognized in polynomial time, and several problems that are hard
 * on other classes of graphs such as minimum vertex coloring or determining
 * maximum cardinality cliques and independent set can be performed in
 * polynomial time when the input is chordal.
 * <p>
 * All methods in this class run in $\mathcal{O}(|V| + |E|)$ time. Determining
 * whether a graph is chordal, as well as computing a perfect elimination order
 * takes $\mathcal{O}(|V| + |E|)$ time, independent of the algorithm
 * ({@link MaximumCardinalityIterator} or {@link LexBreadthFirstIterator}) used
 * to compute the perfect elimination order.
 * <p>
...
public class ChordalityInspector<V, E> {
	 ...
	 *
	 * @param graph          the graph for which a chordality inspector is to be
	 *                       created.
	 ...
	 * Returns a <a href=
	 * "https://en.wikipedia.org/wiki/Chordal_graph#Perfect_elimination_and_efficient_recognition">
	 * perfect elimination order</a> if one exists. The existence of a perfect
	 * elimination order certifies that the graph is chordal. This method returns
	 * null if the graph is not chordal.
	 *
	 * @return a perfect elimination order of a graph or null if graph is not
	 *         chordal.
	 */
	 ...
	 * <a href="http://mathworld.wolfram.com/GraphHole.html">hole</a> (chordless
	 * cycle on 4 or more vertices). The existence of a hole certifies that the
	 * graph is not chordal. This method returns a chordless cycle if the graph is
	 * not chordal, or null if the graph is chordal.
	 *
	 * @return a hole if the {@code graph} is not chordal, or null if the graph is
	 *         chordal.
	 */
	 ...
	 * "https://en.wikipedia.org/wiki/Chordal_graph#Perfect_elimination_and_efficient_recognition">
	 * perfect elimination order</a> with respect to the inspected graph. Returns
	 * false otherwise.
	 *
	 * @param vertexOrder the sequence of vertices of the {@code graph}.
	 * @return true if the {@code graph} is chordal and the vertices in
	 *         {@code vertexOrder} are in perfect elimination order, otherwise
	 *         false.
	 */
	 ...
	 * "https://en.wikipedia.org/wiki/Chordal_graph#Perfect_elimination_and_efficient_recognition">
	 * perfect elimination order</a> with respect to the inspected graph. Returns
	 * false otherwise. Computes a hole if the {@code computeHole} is true.
	 *
	 * @param vertexOrder the sequence of vertices of {@code graph}.
	 * @param computeHole tells whether to compute the hole if the graph isn't
	 *                    chordal.
	 * @return true if the {@code graph} is chordal and the vertices in
	 *         {@code vertexOrder} are in perfect elimination order.
	 */
	...
	/**
	 * Returns a map containing vertices from the {@code vertexOrder} mapped to
	 * their indices in {@code vertexOrder}.
	 *
	...
	private Map<V, Integer> getVertexInOrder(List<V> vertexOrder) {
		Map<V, Integer> vertexInOrder = CollectionUtil.newHashMapWithExpectedSize(vertexOrder.size());
		int i = 0;
		for (V vertex : vertexOrder) {
	...
	/**
	 * Computes a hole from the vertices of {@code subgraph} of the inspected
	 * {@code graph} with vertices {@code a}, {@code b} and {@code c} on this cycle
	 * (there must be no edge between {@code a} and {@code c}.
	 *
	...
	private void findHole(V a, V b, V c) {
		// b is the first vertex in the order produced by the iterator whose
		// predecessors don't form
		// a clique.
		// a and c are a pair of vertices, which are predecessors of b and are not
		// adjacent. These
		// three vertices
		// belong to some chordless cycle in the G[S] where G[S] is a subgraph of G on
		// vertices in
		// S = {u : index_in_order(u) <= index_in_order(v)}.
		// this method uses dfs to find any cycle in G, in which every vertex isn't
		// adjacent to b,
		// except for a and b.
		// then it finds a chordless subcycle in linear time and returns it.

		List<V> cycle = new ArrayList<>(Arrays.asList(a, b, c));
		Map<V, Boolean> visited = CollectionUtil.newHashMapWithExpectedSize(graph.vertexSet().size());
	...
	/**
	 * Computes some cycle in the graph on the vertices from the domain of the map
	 * {@code visited}. More precisely, finds some path from {@code middle} to
	 * {@code finish}. The vertex {@code middle} isn't the endpoint of any chord in
	 * this cycle.
	 *
	 * @param cycle   already computed part of the cycle
	 * @param visited the map that defines which vertex has been visited by this
	 *                method
	 ...
	 * retains first 2 vertices and finds a chordless cycle starting from the third
	 * vertex.
	 *
	...
	/**
	 * Returns the predecessors of {@code vertex} in the order defined by
	 * {@code map}. More precisely, returns those of {@code vertex}, whose mapped
	 * index in {@code map} is less then the index of {@code vertex}.
	 *
	 * @param vertexInOrder defines the mapping of vertices in {@code graph} to
	 *                      their indices in order.
	 * @param vertex        the vertex whose predecessors in order are to be
	 *                      returned.
