...
int main() {
    ...
    // gomory-hu
    void *hu;
    jgrapht_capi_gomoryhu_exec_gusfield(thread, g, &hu);
    assert(jgrapht_capi_error_get_errno(thread) == 0);

    // min s-t cut
    void *source_partition; 
    double value;
    jgrapht_capi_gomoryhu_min_st_cut(thread, hu, 0, 3, &value, &source_partition);
    assert(value == 30.0);
    assert(jgrapht_capi_error_get_errno(thread) == 0);
    ...
    // min cut
    jgrapht_capi_gomoryhu_min_cut(thread, hu, &value, &source_partition);
    assert(value == 30.0);
    assert(jgrapht_capi_error_get_errno(thread) == 0);
    ...
    // tree
    void *tree;
    jgrapht_capi_gomoryhu_tree(thread, hu, &tree);
    int count;
